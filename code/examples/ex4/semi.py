...
def yData(ynn: Function, x: Tensor, para: Any) -> Array:
    ...
    return (6*lam*(pi**2)+1-lam)*sin(pi*x).prod(axis=1).reshape(-1, 1)-3*lam*(sin(pi*x)**3).prod(axis=1).reshape(-1, 1)
def Fx(x: Tensor) -> Array:
    return 6*pi*pi*sin(pi*x).prod(axis=1).reshape(-1, 1)+(sin(pi*x)**3).prod(axis=1).reshape(-1, 1)

def LossPinn(ynn: NN, lapY: NN, pnn: NN, _x: Tensor, para: Any) -> Array:


    _Y=ynn(_x, para['yNet']).reshape(-1, 1)
    return L2Norm(((lapY(_x, para['yNet'])).reshape(-1, 1))+Fx(_x)-((1.0/lam)*pnn(_x, para['pNet']).reshape(-1, 1))-(_Y**3+_Y))
...
def LossP(ynn: Function, pnn: Function, lapP: Function, _x: Tensor, para) -> Array:
    ...
    _Y=ynn(_x, para['yNet']).reshape(-1, 1)
    _P=pnn(_x, para['pNet']).reshape(-1, 1)
    return 12*L2Norm(_laplaceP+ynn(_x, para['yNet']).reshape(-1, 1)-yData(ynn, _x, para)+(3*_Y**2+1)*_P)
...
def LossAll(ynn, pnn, lapY, lapP, paras,key) -> Array:
    _x = random.uniform(key,(MCSizeIn, DimInput))


    return LossPinn(ynn, lapY, pnn, _x, paras)+10*LossP(ynn, pnn, lapP, _x, paras)+alpha*(LossBoundary(ynn, paras['yNet'],key)+10*LossBoundary(pnn, paras['pNet'],key))
...
PFn=jit(lambda _para,key:LossP(ynn,pnn,LapP,random.uniform(key,(MCsizeB,DimInput)),_para))
YFn=jit(lambda _para,key:LossPinn(ynn,LapY,pnn,random.uniform(key,(MCsizeB,DimInput)),_para))
# %%
LstLoss=[0.0]*Epoach
LstJ=[0.0]*Epoach
LstP=[0.0]*Epoach
